...
管道聚合工作在其他聚合输出生成物的地方，而不是直接处理文档，它用于向输出树中添加信息。管道聚合有很多不同的类型，它们为其他聚合处理各种各样的信息，但是这么多的类型可以大致分为两类：

_（父辈）Parent_::
                这类管道聚合包含了父聚合的输出，它可以为已经存在的 bucket 添加新计算的 bucket 或者新的聚合。
_（同辈）Sibling_::
                这一类管道聚合会提供同辈聚合的输出，同时它可以计算一个新的聚合，新的聚合会在作为同辈聚合出现在同一级。
