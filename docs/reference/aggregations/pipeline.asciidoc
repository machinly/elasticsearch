...
_Parent（父辈）_::
                这类 pipeline 聚合包含了 paraent 聚合的输出，它可以为已经存在的 bucket 添加新计算的 bucket 或者新的聚合。
_Sibling（同辈）_::
                这一类 pipeline 聚合会提供 sibling 聚合的输出，同时它可以计算一个新的聚合，新的聚合会在作为 sibling 聚合出现在同一级。
...
`buckets_path` 也可以使用 `_bucket_count` 和多桶聚合的路径，以使该聚合在 pipeline 聚合中返回桶数，而不是返回一个指标。例如，可以在此处使用 `bucket_selector` 过滤掉不包含 terms 聚合的 bucket：
...
* 可能因为一个相关 bucket 的值丢失了，导致正在计算的指标无法生成值。有的 pipeline 聚合必须满足一些特定的要求(例如，由于没有第一个值，无法对第一个指标进行求导，HoltWinters 移动平均线需要 “预热” 数据才能开始计算，等等)

Gap 策略是 pipeline 聚合的一种处理数据缺失的机制。所有 pipeline 聚合都可以接受 "gap_policy" 参数。目前有两种 gap 策略可供选择:
...
_insert_zeros_::
                ...
                proceed as normal.
                此选项将用 (`0`) 替换缺失的值，以保证 pipeline 聚合计算正常进行。
